...
def main(base_path):
    # Inicializar Spark (asegúrate de que Spark esté configurado correctamente)
    import findspark
    findspark.init()
    import pyspark
    from pyspark.sql import SparkSession

    spark = SparkSession.builder.appName("train_spark_mllib_model").getOrCreate()
    ...
    mlflow.set_experiment("flight_delay_experiment")

    with mlflow.start_run():
        # Agregar una columna 'Route' (Ruta)
        features_with_route = features.withColumn(
            'Route', concat(features.Origin, lit('-'), features.Dest)
        )

        # Definir el Bucketizer para 'ArrDelay'
        splits = [-float("inf"), -15.0, 0, 30.0, float("inf")]
        arrival_bucketizer = Bucketizer(splits=splits, inputCol="ArrDelay", outputCol="ArrDelayBucket")

        # Guardar el bucketizer en MLflow
        arrival_bucketizer_path = "{}/models/arrival_bucketizer_2.0.bin".format(base_path)
        arrival_bucketizer.write().overwrite().save(arrival_bucketizer_path)
        mlflow.log_artifact(arrival_bucketizer_path)

        # Aplicar el bucketizer
        ml_bucketized_features = arrival_bucketizer.transform(features_with_route)

        # Aplicar StringIndexer a las columnas categóricas
        index_columns = ["Carrier", "Origin", "Dest", "Route"]
        for column in index_columns:
        ...
        vector_assembler = VectorAssembler(inputCols=numeric_columns + index_columns, outputCol="Features_vec")
        final_vectorized_features = vector_assembler.transform(ml_bucketized_features)
        ...
        accuracy = evaluator.evaluate(model.transform(final_vectorized_features))
